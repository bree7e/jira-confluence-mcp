...
import { preparePage, renderPage, createResolver } from "../dist/import/export-page.js";
...
const resolver = {
  ...
  findPage: async () => "456",
  attachment: async (id, name) => "https://example.com/wiki/download/attachments/" + id + "/" + encodeURIComponent(name),
  jiraUrl: key => "https://example.com/browse/" + key,
};
for (const converter of Object.values(converters)) {
  test(converter.id + ": structure, links, complex tables, macros and metadata", async () => {
    const result = await renderPage(await preparePage(fixture, resolver), converter);
    ...
    assert.match(md, /\[Изображение[^\]]*\]\(https:\/\/example.com\/original.png\)/);
    assert.doesNotMatch(md, /!\[/);
    assert.match(md, /https:\/\/example.com\/wiki\/pages\/viewpage.action\?pageId=456/);
  ...
  test(converter.id + ": attachment resolution and unavailable resources are visible", async () => {
    const page = structuredClone(fixture);
    page.body.storage.value = '<p><ac:image><ri:attachment ri:filename="a b.png"><ri:page ri:content-id="789"/></ri:attachment></ac:image></p>';
    const result = await renderPage(await preparePage(page, resolver), converter);
    assert.match(result.markdown, /attachments\/789\/a%20b.png/);
    const failed = await renderPage(await preparePage(page, { ...resolver, attachment: async () => { throw new Error("404"); } }), converter);
...
test("incremental import, converter change, local conflict and source isolation", async () => {
  ...
  try {
    ...
    assert.equal((await saveImport({ ...result, version: 3, markdown: result.markdown.replace("version: 2", "version: 3") }, dir)).status, "updated");
    assert.equal((await saveImport(result, dir)).status, "conflict");
    assert.equal((await saveImport({ ...result, version: 3, source: "https://other.example/123" }, dir)).status, "conflict");
    const switched = await renderPage(prepared, converters.turndown);
    switched.version = 3;
    ...
    await assert.rejects(saveImport({ ...switched, pageId: "../escape" }, dir), /Invalid page/);
  } finally { await rm(dir, { recursive: true, force: true }); }
});

test("incomplete source rejected; relative attachment links preserve context", async () => {
  await assert.rejects(preparePage({ ...fixture, version: undefined }, resolver), /Incomplete page/);
  const client = new ConfluenceClient("https://example.com/wiki", "", "");
  assert.equal(client.resolveUrl("/download/attachments/1/x.png", true), "https://example.com/wiki/download/attachments/1/x.png");
  assert.equal(client.resolveUrl("/wiki/download/attachments/1/x.png"), "https://example.com/wiki/download/attachments/1/x.png");
...
test("both engines retain entities, nested table content, empty cells and pipe characters", async () => {
  const page = structuredClone(fixture);
  page.body.storage.value = '<h1>Тест</h1><p>до&nbsp;после &copy; &lt;ok&gt;</p><table><tr><th>A</th><th>B</th></tr><tr><td></td><td><code>a|b</code></td></tr><tr><td><table><tr><td>Внутренняя</td></tr></table></td><td>Хвост</td></tr></table>';
  for (const converter of Object.values(converters)) {
    const result = await renderPage(await preparePage(page, resolver), converter);
...
test("unmanaged files and corrupt manifests are never overwritten", async () => {
  const dir = await mkdtemp(join(tmpdir(), "confluence-import-"));
  try {
    const result = await renderPage(await preparePage(fixture, resolver), converters.remark);
...
test("heading normalization and original root links are independent of storage heading level", async () => {
  const page = structuredClone(fixture);
  page.body.storage.value = '<h3>Раздел</h3><h4>Подраздел</h4><p><a href="/browse/IDPPA-1">Задача</a></p>';
  const client = new ConfluenceClient("https://example.com/wiki/", "", "");
  for (const converter of Object.values(converters)) {
    const result = await renderPage(await preparePage(page, createResolver(client, client.sourceBaseUrl)), converter);
...
test("empty cells preserve the number of table columns in both engines", async () => {
  const page = structuredClone(fixture);
  page.body.storage.value = '<table><tr><th></th><th>B</th><th>C</th></tr><tr><td></td><td>x</td><td></td></tr></table>';
  for (const converter of Object.values(converters)) {
    const result = await renderPage(await preparePage(page, resolver), converter);
    const rows = result.markdown.split("\n").filter(line => line.startsWith("|"));
    assert.equal(rows.length, 3);
    for (const row of rows) assert.equal(row.match(/(?<!\\)\|/g).length, 4, converter.id + ": " + row);
  }
});

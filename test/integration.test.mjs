...
test("CLI import, repeat and HTTP 404 preserve imports; no Jira credentials needed", async () => {
...
test("MCP text export and disk import share the same implementation", async () => {
  ...
  const client = {
    sourceBaseUrl: "https://example.com/wiki",
    getPage: async () => fixture,
  ...
  try {
    ...
    assert.equal(
      await readFile(join(dir, "123.md"), "utf8"),

...
- Простые таблицы становятся GFM-таблицами. Сложные ячейки выносятся после таблицы в блоки
  «Таблица N, строка R, столбец C» с явными HTML-якорями и Markdown-ссылками на них.
  Объединённые ячейки повторяются по занимаемым строкам/столбцам с предупреждением.
  Таблицам без отдельной строки заголовков добавляются нейтральные заголовки «Столбец N».
